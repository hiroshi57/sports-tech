...
def compute_sprint_score(seq: PoseSequence) -> float:
    """
    スプリントスコア: 腰中心の最大移動速度とケイデンス（脚の上下動頻度）から算出。
    ...
    return round(_clamp(speed_component * 0.6 + cadence_component * 0.4), 1)


def compute_body_usage_score(seq: PoseSequence) -> float:
    """
    身体の使い方スコア: 体幹の安定性（肩-腰の傾きの一貫性）と左右対称性から算出。
    ...
    return round(_clamp(stability_component * 0.6 + symmetry_component * 0.4), 1)
...
def compute_positioning_score(seq: PoseSequence) -> float:
    ...
    return round(_clamp(coverage_component * 0.5 + distance_component * 0.5), 1)
